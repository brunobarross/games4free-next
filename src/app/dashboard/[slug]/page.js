...
const page = async ({params}) => {
  ...
  const jogosFiltrados = await  jogos.filter((jogo) => {
    const plataformas = ['Epic Games Store', 'Steam', 'GOG', 'Playstation 4', 'Xbox 360', 'Xbox One', 'Nintendo Switch', 'Ubisoft', 'Android']
    if(params.slug === 'games') {
      return jogo.type === 'Game' && plataformas.some((plataforma) => jogo.platforms.includes(plataforma))
    } else if(params.slug === 'dlcs' && plataformas.some((plataforma) => jogo.platforms.includes(plataforma))) {
  ...
  return (
    <div className="container md:mt-16 mt-32 px-12 mb-12">
      <Navbar />
